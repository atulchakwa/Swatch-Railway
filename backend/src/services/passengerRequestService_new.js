...
class PassengerRequestService {
  ...
  async createFromTransmitter(data) {
    ...
    await db.collection(this.collection).doc(requestRef.id).set(requestData);
  ...
  async handleDeviceEvent(requestId, eventData) {
    ...
    if (requestData.assignedWorkerId !== eventData.pressedBy) {
    ...
    if (eventData.buttonPressed === 'CLEANER_RESET' && requestData.assignedWorkerRole !== 'JANITOR') {
      throw new ValidationError('Only janitor can press cleaner reset');
    }
    if (eventData.buttonPressed === 'ATTENDANT_RESET' && requestData.assignedWorkerRole !== 'ATTENDANT') {
      throw new ValidationError('Only attendant can press attendant reset');
    }

    if (eventData.buttonPressed === 'CLEANER_RESET' || eventData.buttonPressed === 'ATTENDANT_RESET') {
      if (requestData.status === 'PENDING' || requestData.status === 'ACCEPTED') {
        await requestRef.update({
          status: 'IN_PROGRESS',
          startedAt: new Date().toISOString(),
          startedBy: eventData.pressedBy,
          deviceButtonPressed: eventData.buttonPressed,
          statusHistory: admin.firestore.FieldValue.arrayUnion({
            status: 'IN_PROGRESS',
            timestamp: new Date().toISOString(),
            actor: 'DEVICE',
            buttonPressed: eventData.buttonPressed
          }),
          updatedAt: new Date().toISOString()
        });
        return { status: 'IN_PROGRESS', message: `Task started via ${eventData.buttonPressed} button` };
        ...
        await requestRef.update({
          ...
          completedBy: eventData.pressedBy,
          deviceButtonPressed: eventData.buttonPressed,
          durationSec,
        ...
        return { status: 'COMPLETED', durationSec, message: `Task completed via ${eventData.buttonPressed} button` };
      }
    }
  ...
  async getWorkerRequests(userId, query = {}) {
    ...
    const result = await paginate(q, { limit: parseInt(limit), cursor });
  ...
  async getAllRequests(query = {}) {
    ...
    let q = db.collection(this.collection).orderBy('passengerPressedAt', 'desc');

    if (status) q = q.where('status', '==', status);
    if (trainNumber) q = q.where('trainNumber', '==', trainNumber);
    if (requestType) q = q.where('requestType', '==', requestType);
    if (startDate) q = q.where('passengerPressedAt', '>=', new Date(startDate));
    if (endDate) q = q.where('passengerPressedAt', '<=', new Date(endDate));
  ...
  async getTimingAnalytics(query = {}) {
    ...
    return {
      ...
      avgReactionTimeMs: requests.length > 0 ? Math.round(requests.reduce((sum, r) => sum + (r.acceptedAt ? new Date(r.acceptedAt).getTime() - new Date(r.passengerPressedAt).getTime() : 0), 0) / requests.length) : 0,
      avgArrivalTimeMs: requests.length > 0 ? Math.round(requests.reduce((sum, r) => sum + (r.startedAt ? new Date(r.startedAt).getTime() - new Date(r.acceptedAt || r.passengerPressedAt).getTime() : 0), 0) / requests.length) : 0,
      avgTaskDurationSec: requests.filter(r => r.status === 'COMPLETED').length > 0 ? Math.round(requests.filter(r => r.completedAt && r.startedAt).reduce((sum, r) => sum + (new Date(r.completedAt).getTime() - new Date(r.startedAt).getTime()) / 1000, 0) / requests.filter(r => r.status === 'COMPLETED').length) : 0,
      avgResolutionTimeSec: completedCount > 0 ? Math.round(requests.filter(r => r.status === 'COMPLETED').reduce((sum, r) => sum + (new Date(r.completedAt).getTime() - new Date(r.passengerPressedAt).getTime()) / 1000, 0) / completedCount) : 0,
