...
class StationReportRepository {
  ...
  static String _typeToRouteSuffix(String type) {
    ...
    var suffix = type;
    if (suffix.startsWith('daily_'))
      suffix = suffix.substring(6);
    else if (suffix.startsWith('monthly_'))
  ...
  static Future<StationReport> generateDaily(
    String type,
    String stationId,
    String date,
  ) async {
    ...
    final res = await http.post(
      Uri.parse('$baseUrl/api/station-reports/daily/$route'),
      headers: await _headers(),
      body: jsonEncode({'stationId': stationId, 'date': date}),
    );
    if (res.statusCode == 200 || res.statusCode == 201)
      return StationReport.fromJson(jsonDecode(res.body));
    throw Exception('Failed to generate daily report');
  }

  static Future<StationReport> generateMonthly(
    String type,
    String stationId,
    int month,
    int year,
    ...
    final route = _typeToRouteSuffix(type);
    final res = await http.post(
      Uri.parse('$baseUrl/api/station-reports/monthly/$route'),
      headers: await _headers(),
      body: jsonEncode({'stationId': stationId, 'month': month, 'year': year}),
    );
    if (res.statusCode == 200 || res.statusCode == 201)
      return StationReport.fromJson(jsonDecode(res.body));
  ...
  static Future<StationReport> generateRange(
    String type,
    String stationId,
    String startDate,
    String endDate,
    ...
    final res = await http.post(
      Uri.parse('$baseUrl/api/station-reports/range'),
      headers: await _headers(),
      body: jsonEncode({
        'stationId': stationId,
        'reportType': type,
    ...
    if (res.statusCode == 200 || res.statusCode == 201) {
      ...
      return StationReport.fromJson(body['report'] ?? body);
    }
    throw Exception('Failed to generate range report');
  }

  static Future<StationReport> generateArchiveRetrieval(
    String stationId,
    String startDate,
    String endDate,
  ) async {
    final res = await http.post(
      Uri.parse('$baseUrl/api/station-reports/archive-retrieval'),
      headers: await _headers(),
      body: jsonEncode({
        'stationId': stationId,
        'startDate': startDate,
  ...
  static Future<void> schedule(
    ...
    final res = await http.post(
      Uri.parse('$baseUrl/api/station-reports/schedule'),
      headers: await _headers(),
      body: jsonEncode({
    ...
    if (res.statusCode != 201 && res.statusCode != 200)
      throw Exception('Failed to schedule report');
  }

  static Future<List<Map<String, dynamic>>> listSchedules() async {
    final res = await http.get(
      Uri.parse('$baseUrl/api/station-reports/schedules'),
      headers: await _headers(),
    );
    if (res.statusCode == 200) {
  ...
  static Future<void> deleteSchedule(String uid) async {
    final res = await http.delete(
      Uri.parse('$baseUrl/api/station-reports/schedule/$uid'),
      headers: await _headers(),
    );
  ...
  static Future<Map<String, dynamic>> getScoreTrend(String stationId) async {
    final res = await http.get(
      Uri.parse(
        '$baseUrl/api/station-reports/score-trend?stationId=$stationId',
      ),
      headers: await _headers(),
    );
  ...
  static Future<Map<String, dynamic>> generateAuditReport(
    ...
    final uri = Uri.parse(
      '$baseUrl/api/station-reports/audit/$type',
