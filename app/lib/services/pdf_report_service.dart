...
class PDFReportService {
      ...
      pw.MultiPage(
        ...
        build: (pw.Context context) {
          ...
          if (scalarEntries.isNotEmpty) {
            ...
            final gridEntries = scalarEntries.length <= 12 ? scalarEntries : scalarEntries.sublist(0, 12);
          ...
          if (scalarEntries.length > 12) {
            ...
            widgets.add(_buildSectionHeader('Additional Details', color: const PdfColor.fromInt(0xff2c7fb8)));
  ...
  static String _reportReference(StationReport report) {
    final slug = report.reportType.replaceAll(RegExp(r'^(daily|monthly)_'), '').toUpperCase();
  ...
  static pw.Widget _buildReportHeader(pw.ImageProvider logo1, pw.ImageProvider logo2, String title, String reference, String timestamp) {
    return pw.Container(
      decoration: pw.BoxDecoration(
        color: PdfColors.white,
        border: pw.Border.all(color: primaryColor, width: 1.5),
        ...
        children: [
          pw.Container(
            ...
            child: pw.Row(
              mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
              children: [
                pw.Text('SWATCH RAILWAY', style: pw.TextStyle(color: PdfColors.white, fontSize: 9, fontWeight: pw.FontWeight.bold)),
                pw.Text('STATION CLEANING MODULE', style: pw.TextStyle(color: PdfColors.white, fontSize: 9, fontWeight: pw.FontWeight.bold)),
                pw.Text(reference, style: pw.TextStyle(color: PdfColors.white, fontSize: 8)),
              ],
            ),
          ),
          pw.Padding(
              ...
              children: [
                ...
                pw.Expanded(
                  child: pw.Column(
                    ...
                    children: [
                      ...
                      pw.SizedBox(height: 2),
                      pw.Text('Indian Railways - Station Cleaning Enterprise Monitoring System',
                      ...
                      pw.Container(
                        ...
                        child: pw.Text('Status: SYSTEM GENERATED • Digitally Verified', style: pw.TextStyle(fontSize: 8, color: primaryColor, fontWeight: pw.FontWeight.bold)),
                      ),
                    ],
                  ),
                ),
                pw.Column(
                  ...
                  children: [
                    ...
                    pw.Text(timestamp, style: pw.TextStyle(fontSize: 8, color: PdfColors.grey800, fontWeight: pw.FontWeight.bold)),
                  ],
                ),
              ],
            ),
      ...
      children: [
        pw.TableRow(
          ...
          children: [
            ...
            _metaCell('Generated By', report.generatedByName.isNotEmpty ? report.generatedByName : report.generatedBy),
          ],
        ),
        pw.TableRow(
  ...
  static pw.Widget _metaCell(String label, String value) {
    return pw.Padding(
      ...
      child: pw.Column(
        crossAxisAlignment: pw.CrossAxisAlignment.start,
        children: [
          pw.Text(label.toUpperCase(), style: pw.TextStyle(fontSize: 6.5, color: PdfColors.grey600, fontWeight: pw.FontWeight.bold)),
          pw.SizedBox(height: 2),
          pw.Text(value.isEmpty ? 'N/A' : value, style: pw.TextStyle(fontSize: 8.5, color: PdfColors.grey900, fontWeight: pw.FontWeight.bold)),
        ],
      ),
    );
  }
  ...
  static pw.Widget _buildDataTable(List<String> keys, List<dynamic> records, {bool isFeedback = false}) {
    ...
    for (var i = 0; i < records.length; i++) {
      ...
      rows.add(pw.TableRow(decoration: pw.BoxDecoration(color: i.isEven ? PdfColors.white : PdfColors.grey50), children: cells));
    }

    return pw.Table(
      border: pw.TableBorder.all(color: borderColor, width: 0.5),
      columnWidths: {for (var i = 0; i < keys.length; i++) i: pw.FlexColumnWidth(1)},
      children: [
        pw.TableRow(
          decoration: pw.BoxDecoration(color: PdfColors.teal),
          children: keys.map((k) => pw.Padding(
            padding: const pw.EdgeInsets.all(5),
            child: pw.Text(_formatLabel(k),
